...
core.mkPackage {
  ...
  let
    inherit (std.strings) concatStringsSep concatMapStringsSep replaceStrings;
    ...
    objects = map (x: replaceStrings [ ".c" ] [ ".o" ] (builtins.baseNameOf x)) sources;
  in
  runCommand.onHost "${name}-${version}"
    {
      tools = [ tinycc-mes.onBuildForHost.compiler ];
    }
    ''
      # Unpack
      ...
      # Build
      alias CC="tcc -B ${tinycc-mes.onBuildForHost.libs}/lib ${concatStringsSep " " CFLAGS}"
      ${concatMapStringsSep "\n" (f: "CC -c ${f}") sources}

      # Link
      CC -o patch ${concatStringsSep " " objects}

      # Check
      # NOTE: How to fix this with cross-compilation?
      ./patch --version
