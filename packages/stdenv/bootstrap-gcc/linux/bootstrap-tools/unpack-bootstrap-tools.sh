...
#
# (GrizzlT) most likely performed to avoid problems when patching itself
...
# TODO: use a static patchelf
LD_LIBRARY_PATH=$out/lib $LD_BINARY $out/bin/cp $out/lib/libgcc_s.so.1 .
# Point to all lib files (build directory, output drv, libstdc++)
#
# DT_RPATH is checked before LD_LIBRARY_PATH, this makes sure the bootstrapped
# files work by themselves and are isolated.
#
# TODO: the current patchelf version only requires libm and libc
# Check this for the other platforms -> currently not on x86_64-linux
...
for i in $out/lib/librt* $out/lib/libpcre*; do
    if [ -L "$i" ]; then continue; fi
    echo patching "$i"
